...
- Leaf litter piles form on the ground beneath leaf canopies in autumn only, as part of processing columns beneath canopies. They are updated wherever snow would thaw: placed in autumn and removed in other seasons. Where snow accumulates instead, snow replaces them, so a pile can remain into winter until the first snowfall covers it.
    ...
    - Each dimension with seasons records its previous season and the season time at which the current season began. When a chunk catches up, a column has a pile if its latest thaw-processing tick fell in autumn, and none otherwise; it is changed only when the latest thaw-processing timestamp is newer than the chunk's stored update time.
