...
- Optionally (configurable for performance, enabled by default), columns beneath leaf canopies are processed as well. While disabled, nothing beneath canopies changes, and snow accumulation, icicles and leaf piles there are all inactive.
    - Optionally (configurable, enabled by default), snow also accumulates on the ground beneath leaf canopies and water there freezes. Both thaw there under the same conditions as elsewhere, also while accumulation is disabled.
    - Optionally (configurable, enabled by default), where snow would accumulate beneath a canopy, in a configurable fraction of columns (default 0.02, chosen pseudo-randomly by x/z and the world seed) an icicle forms in every air block directly below leaves that is not lit by a block light of 10 or more (the same limit as snow), hanging from them. Icicles thaw under the same conditions as snow, also while icicle formation is disabled, break when the leaves above are gone, and drop nothing when broken.
- Leaf litter piles form on the ground beneath leaf canopies in autumn only, as part of processing columns beneath canopies. They are updated wherever snow would thaw: placed in autumn and removed in other seasons. Where snow accumulates instead, snow replaces them, so a pile can remain into winter until the first snowfall covers it.
    - Only leaves on a configurable list produce piles (default: vanilla oak, birch, acacia and dark oak leaves); each listed leaf type has its own pile block.
    - A pile looks like a few scattered, almost flat patches of its parent leaves with gaps between them, laid out pseudo-randomly per position so each pile looks different.
    - Piles form only in air, and snow replaces them.
    - Optionally (configurable, enabled by default), piles form in a configurable fraction of columns beneath a canopy (default 0.2), chosen pseudo-randomly by x/z and the world seed like icicle columns but independent of them, so the two select different columns.
    - For each position in the repeating pattern, the dimension records whether its latest thaw-processing tick outside winter occurred in autumn. When a chunk catches up, a column has a pile if its relevant latest thaw-processing tick was that autumn event, and none otherwise. For columns that only thaw outside winter, a pile from autumn therefore survives a winter without snowfall; it is changed only when the relevant thaw-processing timestamp is newer than the chunk's stored update time.
...
- Whenever the global raining state starts or stops, or the current schedule reaches its end, a new randomized processing schedule is generated and reset to its first step.
