...
- Leaf litter piles form on the ground beneath leaf canopies in autumn only, as part of processing columns beneath canopies. They are updated wherever snow would thaw: placed in autumn and removed in other seasons. Where snow accumulates instead, snow replaces them, so a pile can remain into winter until the first snowfall covers it.
    ...
    - A pile looks like a single snow layer textured with its parent leaves.
    - Piles form only in air, and snow replaces them.
    - Optionally (configurable, enabled by default), piles form in a configurable fraction of columns beneath a canopy (default 0.02), chosen pseudo-randomly by x/z and the world seed like icicle columns but independent of them, so the two select different columns.
